...
class IsBank : IBank
{
    public IsBank(string userCode, string password)
    {
        _userCode = userCode;
        _password = password;
    }
...
class IsBankFactory : IBankFactory
{
    public IBank CreateInstance()
    {
        IsBank isBank = new("userid", "ispwd1");
        return isBank;
    }
}
...
class BankCreator
{
    public IBank Create(BankType bankType)
    {
        ...
        //....Factory classlarını reflection ile dinamik bir şekilde oluşturuyoruz. burada BankCreator ın, Factory lere bağımlılığını kaldırmış olduk.
